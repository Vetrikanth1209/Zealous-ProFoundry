...
const SERVICE_PORT = parseInt(process.env.PORT, 10) || 9000; // Default to 9000 if PORT is not set

// Create a Consul client
const consul = new Consul({
    host: CONSUL_HOST,
    port: CONSUL_PORT,
    secure: true, // Use HTTPS since Render provides an HTTPS endpoint
    promisify: true,
});
...
const registerService = async () => {
    try {
        ...
        await consul.agent.service.register({
            ...
            port: SERVICE_PORT, // The actual service running port
            check: {
                http: `https://${CONSUL_HOST}/v1/agent/checks`, // Use HTTPS for Consul health checks
                interval: '10s',
                timeout: '5s',
            },
        });
